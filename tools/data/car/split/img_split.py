...
def crop_and_save_img(info, windows, window_anns, img_dir, no_padding,
    ...
    for i in range(windows.shape[0]):
        ...
        try:
            ...
            print(osp.join(img_dir, info['filename']))
...
def _load_dota_single(imgfile, img_dir, ann_dir):
    ...
    try:
        ...
        content.update(
            dict(width=0, height=0, filename=imgfile, id=img_id))
...
def main(input_path,save_path):
    ...
    json_file = '/data5/laiping/tianzhibei/code/Large-Selective-Kernel-Network/tools/data/car/split/split_configs/ss_test.json'
