...
from tools import retreivel_tools, crop_image, classify_tool, evaluation_tool, evaluation_iou
# from tools import retreivel_tools,crop_image,classify_tool,evaluation_tool,evaluation_iou,evaluation_iou_dota
...
def parse_args():
    ...
    parser.add_argument('--gallery_gt', type=str, default=False)
    parser.add_argument('--xml_output_path', type=str, default="")
    parser.add_argument('--refine_output_path', type=str, default="")
    parser.add_argument('--checkpoint', help='checkpoint file')

    parser.add_argument('--config', help='test config file path')

    parser.add_argument('--gallery_crop_path', type=str, default=False)
    parser.add_argument('--query_crop_path', type=str, default=False)
    parser.add_argument('--query_path', type=str, default=False)
    parser.add_argument('--query_gt', type=str, default=False)
...
def fairv2_test(args):
    ...
    # save_dirs = "/data5/laiping/tianzhibei/test-wlsknet-12-18-all/test/"

    start_time = time.time()
    try:
        img_split_new_auto.main(args.gallery_path, args.save_dirs)
    except:
        print("The image has been processed!")
    print(args)
    ...
    cfg.data_root = args.save_dirs
    cfg.data.test['ann_file'] = f"{args.save_dirs}/images/"
    cfg.data.test['img_prefix'] = f"{args.save_dirs}/images/"
...
def check_and_create_paths(paths):
    for path in paths:
...
def main():
    ...
    print(f"[Summary] Inference Report")
